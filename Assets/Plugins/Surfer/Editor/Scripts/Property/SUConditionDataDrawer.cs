    ...
    public class SUConditionDataDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.propertyType == SerializedPropertyType.Generic)
            {
                ...
                _stateNames = DefaultConditionChecks.UnifiedNames;
                ...
                {
                    _idsIndex = Mathf.Clamp(System.Array.IndexOf(_stateNames,ConditionChecksExtensions.GetNameFromUnion(_key.stringValue)),0,_stateNames.Length);
                }


                position.height = SurferHelper.lineHeight;
                _idsIndex = EditorGUI.Popup(position,!string.IsNullOrEmpty(label.text) ? "Condition" : label.text,_idsIndex,_stateNames);

                _name.stringValue =  _stateNames[_idsIndex];
                _key.stringValue =  ConditionChecksExtensions.GetKeyFromUnion(_stateNames[_idsIndex]);
